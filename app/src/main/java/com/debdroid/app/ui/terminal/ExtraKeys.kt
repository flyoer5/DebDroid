...
fun performExtraKey(view: TerminalView?, key: ExtraKey, state: ExtraKeysState) {
    android.util.Log.d("DD-TRACE", "performExtraKey key=$key view=${view != null} mTermSession=${view?.mTermSession != null} emulator=${view?.mTermSession?.emulator != null} pid=${view?.mTermSession?.pid}")
    view ?: return
    when (key) {
